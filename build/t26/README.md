...
檢查邊界：腳本會回查凍結來源的檔案、行號與字串；一般 fixture 列維持行號＋needle 精確比對，對會持續編輯的活正典則可用 class 後綴 `-needle-primary` 指定 needle 全檔存在為主錨、行號僅作診斷提示，避免插刪行造成誤紅，同時仍在 needle 真正消失時 fail-closed。腳本另會雙向核對 v1.8 的字面標記，並驗證每個 DTC 只有一個含五項必要欄位的章節。README 自然語言的「CI-deferred／pre-CI」分類仍是人工盤點後凍結的 fixture；腳本能偵測目錄狀態與表格計數漂移，不能自行理解自然語言並證明人工分類絕無遺漏。
